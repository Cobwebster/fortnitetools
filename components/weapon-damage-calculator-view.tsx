...
export function WeaponDamageCalculatorView() {
  const t = useTranslations('tools.weaponDamage')
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
          ...
          <div className="lg:col-span-3 flex flex-col gap-5">
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              <h2 className="text-sm font-bold uppercase tracking-wider text-foreground mb-4">{t('targetOptionsTitle')}</h2>

              {/* HP preset */}
              <div className="mb-4">
                <p className="text-xs font-semibold text-muted-foreground mb-2">{t('targetHp')}</p>
                ...
                <div className="mt-2">
                  <input
                    ...
                    placeholder={t('customHpPlaceholder')}
              ...
              >
                <span className="text-sm font-medium text-foreground">{t('headshotDamage')}</span>
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            ...
            <div className={`rounded-xl border p-5 ${RARITY_BG[weapon.rarity]}`}>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: t('baseDamage'), val: weapon.dmg },
                  { label: t('hsMultLabel'), val: `×${weapon.hs}` },
                  { label: t('fireRateLabel'), val: t('fireRateValue', { rate: weapon.fireRate }) },
                  { label: t('magSizeLabel'), val: weapon.mag },
                ].map(({ label, val }) => (
            ...
            {/* Kill stats */}
            <div className="rounded-xl border border-border bg-card p-5">
              <p className="text-xs font-bold uppercase tracking-wider text-primary mb-4">{t('vsPlayer', { hp })}</p>
              <div className="flex flex-col gap-3">
                <div className="flex justify-between items-baseline">
                  <span className="text-sm text-muted-foreground">{t('damagePerShot')}</span>
                  <span className="font-display text-2xl font-bold text-foreground">{results.effectiveDmg.toFixed(1)}</span>
                </div>
                <div className="flex justify-between items-baseline">
                  <span className="text-sm text-muted-foreground">{t('shotsToKill')}</span>
                  <span className={`font-display text-2xl font-bold ${results.magCanKill ? 'text-primary' : 'text-destructive'}`}>
                    {results.shotsToKill}
                  </span>
                </div>
                <div className="flex justify-between items-baseline">
                  <span className="text-sm text-muted-foreground">{t('timeToKillLabel')}</span>
                  <span className="font-display text-2xl font-bold text-foreground">{t('timeToKillValue', { seconds: results.timeToKill.toFixed(2) })}</span>
                </div>
                <div className="flex justify-between items-baseline">
                  <span className="text-sm text-muted-foreground">{headshot ? t('dpsHeadshot') : t('dpsBody')}</span>
                  <span className="font-display text-2xl font-bold text-foreground">{results.effectiveDps.toFixed(1)}</span>
                </div>
                {headshot && (
                  <div className="flex justify-between items-baseline text-xs">
                    <span className="text-muted-foreground">{t('bodyDpsReference')}</span>
                    <span className="text-muted-foreground">{results.bodyDps.toFixed(1)}</span>
                  </div>
                )}
                {!results.magCanKill && (
                  <div className="rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
                    {t('needsReload', { mag: weapon.mag, dmg: weapon.mag * weapon.dmg })}
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Structure damage */}
            <div className="rounded-xl border border-border bg-card p-5">
              <p className="text-xs font-bold uppercase tracking-wider text-foreground mb-3">{t('structureTitle')}</p>
              <div className="flex flex-col gap-2">
                {[
                  { label: t('structureWood'), shots: results.shotsToBreakWood,  color: 'text-amber-600' },
                  { label: t('structureBrick'), shots: results.shotsToBreakBrick, color: 'text-stone-400' },
                  { label: t('structureMetal'), shots: results.shotsToBreakMetal, color: 'text-cyan-400' },
                ].map(({ label, shots, color }) => (
                  <div key={label} className="flex justify-between items-center">
                    <span className={`text-xs font-medium ${color}`}>{label}</span>
                    <span className="text-sm font-bold text-foreground">{t('shotsUnit', { shots })}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="mt-6 rounded-xl border border-border bg-card p-6">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm leading-relaxed text-muted-foreground mb-2">{t('howP1')}</p>
