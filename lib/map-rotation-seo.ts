...
export const MAP_ROTATION_FAQS: { question: string; answer: string }[] = [
  {
    ...
    answer:
      'The live map depends on the current minute of the hour. Venture is live from :00 to :20, Oasis from :20 to :40, and Elite Stronghold from :40 to :00. The timer at the top of this page shows the active island and the exact minutes and seconds remaining.',
  },
  {
  ...
  },
  {
    ...
    answer:
      'The current Reload pool is Venture (Chapter 1 island), Oasis (desert theme), and Elite Stronghold. Older islands such as Slurp Rush, Squid Grounds, Surf City, and Nitemare Island have been in the rotation before and can return in later updates.',
  },
  {
    question: 'Is the Reload map rotation the same in every region?',
    answer:
      'Yes. The rotation is synchronised globally to clock minutes, so when Venture is live in NA East it is live in EU and OCE too. Minutes past the hour are the same in every timezone, so the schedule works no matter where you play.',
  },
  {
...
export const PAST_RELOAD_MAPS: { name: string; note: string }[] = [
  { name: 'Slurp Rush', note: 'Chapter 2 Slurp theme with Slurpy Swamp-style POIs' },
